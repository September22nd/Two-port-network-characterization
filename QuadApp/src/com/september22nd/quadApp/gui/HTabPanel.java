	...
	public HTabPanel(CQuad quad) {
		...
		try {
			String s11 = (displayForm == DisplayForm.RECT) ?
					String.format("%s", this.quad.getHybridParams().get(0, 0).toRectString()):
					String.format("%s", this.quad.getHybridParams().get(0, 0).toPolarString(angUnit));
			String s12 = (displayForm == DisplayForm.RECT) ?
					String.format("%s", this.quad.getHybridParams().get(0, 1).toRectString()):
					String.format("%s", this.quad.getHybridParams().get(0, 1).toPolarString(angUnit));
			String s21 = (displayForm == DisplayForm.RECT) ?
					String.format("%s", this.quad.getHybridParams().get(1, 0).toRectString()):
					String.format("%s", this.quad.getHybridParams().get(1, 0).toPolarString(angUnit));
			String s22 = (displayForm == DisplayForm.RECT) ?
					String.format("%s", this.quad.getHybridParams().get(1, 1).toRectString()):
					String.format("%s", this.quad.getHybridParams().get(1, 1).toPolarString(angUnit));
			value11.setText(s11);
			value12.setText(s12);
			value21.setText(s21);
			value22.setText(s22);
		...
		if (!isDNE) {
			...
			case 4:
				...
				s = (displayForm == DisplayForm.RECT) ?
						String.format("%s", Complex.prod(this.quad.getHybridParams().get(i, j), factor).toRectString()):
						String.format("%s", Complex.prod(this.quad.getHybridParams().get(i, j), factor).toPolarString(angUnit));
				break;
			case 3:
